<%@ page language="java" contentType="text/html; charset=utf-8"
	pageEncoding="utf-8"%>
<!DOCTYPE html>
<html>

<head>
    <meta charset="utf-8">
    <title>register</title>
    <script src="./js/jquery-3.3.1.min.js"></script>
    <script src="./js/bootstrap.min.js"></script>
    <link href="./css/bootstrap.min.css" rel="stylesheet">
</head>

<body>
    <div class="row">
        <div class="col-lg-8 col-md-offset-2">
            <div class="panel panel-success">
                <!--标题-->
                <div class="panel-heading">
                    <h1>注册</h1>
                </div>
                <div class="panel-body">
                    <div class="row center-block">
                        <!--注册信息表单-->
                        <div class="col-md-6">
                            <form class="form-horizontal" method="post" action="userRegisterServlet" role="form" id="register">
                                <!--账号-->
                                <div class="form-group col-md-12">
                                    <div class="col-md-8 col-md-offset-2 input-group">
                                        <span class="input-group-addon">
                                            <label class="control-label" for="userName">账号</label>
                                        </span>
                                        <input type="text" id="userName" name="userName" class="form-control" placeholder="请输入账号" onchange="checkusername()" maxlength="20">
                                        <span class="glyphicon glyphicon-certificate form-control-feedback" style="color:red;"></span>
                                    </div>
                                    <div class="col-md-8 col-md-offset-2">
                                        <span class="help-block text-left" id="userNameHelp"></span>
                                    </div>
                                </div>
                                <!--密码-->
                                <div class="form-group col-md-12">
                                    <div class="input-group col-md-8 col-md-offset-2">
                                        <span class="input-group-addon">
                                            <label class="control-labelv" for="password">密码</label>
                                        </span>
                                        <input type="password" id="password" name="password" class="form-control" placeholder="请输入密码" onchange="checkuserpass()" maxlength="20">
                                        <span class="glyphicon glyphicon-certificate form-control-feedback" style="color:red;"></span>
                                    </div>
                                    <div class="col-md-8 col-md-offset-2">
                                        <span class="help-block text-left" id="passwordHelp"></span>
                                    </div>
                                </div>
                                <!--手机-->
                                <div class="form-group col-md-12">
                                    <div class="input-group col-md-8 col-md-offset-2">
                                        <span class="input-group-addon">
                                            <label class="control-label" for="phone">手机</label>
                                        </span>
                                        <input type="tel" id="phone" name="phone" class="form-control" placeholder="请输入有效11位号码" onchange="checkphone()" maxlength="11">
                                        <span class="glyphicon glyphicon-certificate form-control-feedback" style="color:red;"></span>
                                    </div>
                                    <div class="col-md-8 col-md-offset-2">
                                        <span class="help-block text-left" id="phoneHelp"></span>
                                    </div>
                                </div>
                                <!--昵称-->
                                <div class="form-group col-md-12">
                                    <div class="input-group col-md-8 col-md-offset-2">
                                        <span class="input-group-addon">
                                            <label class="control-label" for="name">昵称</label>
                                        </span>
                                        <input type="text" id="name" name="name" class="form-control" placeholder="请输入昵称" onchange="checknickname()" maxlength="20">
                                        <span class="glyphicon glyphicon-certificate form-control-feedback" style="color:red;"></span>
                                    </div>
                                    <div class="col-md-8 col-md-offset-2">
                                        <span class="help-block text-left" id="nameHelp"></span>
                                    </div>
                                </div>
                                <!--已注册跳转-->
                                <div class="form-group col-md-12 text-center">
                                    <a href="./index.jsp">已注册用户，请点击进入登陆。</a>
                                </div>
                            </form>
                        </div>
                        <!--LOGO-->
                        <div class="col-md-4">
                            <img src="src/Logo.png" class="img-responsive" alt="LOGO">
                        </div>
                        <!--表单按钮-->
                        <div class="row">
                            <div class="modal-footer center-block col-md-12">
                                <input type="button" id="btn" class="btn btn-success col-md-4 col-md-offset-2" value="注册" name="registersubmit" onclick="register()">
                                <input type="button" id="btn-cancl" class="btn btn-default col-md-4" data-dismiss="modal" value="取消" onclick="cnaclRegister()">
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
<script src="./js/register.js" type="text/javascript"></script>
</body>

</html>
